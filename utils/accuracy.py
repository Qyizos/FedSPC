...
def compute_metrics(pred, gt, names):
    ...
    """
    # 实例化格式，用于计算object-level metrics
    gt_labeled = label(gt)              # ndarray, [1000, 1000]
    pred_labeled = label(pred)          # ndarray, [1000, 1000]
    # 显著性格式，用于计算pixel-level metrics
    gt_binary = gt_labeled.copy()
    pred_binary = pred_labeled.copy()
    gt_binary[gt_binary > 0] = 1        # 将GT转为显著性分割格式， 计算指标
    pred_binary[pred_binary > 0] = 1    # 将预测转为显著性分割格式，计算指标
    gt_binary, pred_binary = gt_binary.flatten(), pred_binary.flatten() # 将其展平，1,000 * 1,000-> 1,000,000
    ...
    if 'haus' in names:
        results['dice'], results['p_iou'], results['haus'] = accuracy_object_level(pred_labeled, gt_labeled, True)
    elif 'dice' in names or 'p_iou' in names:
        results['dice'], results['p_iou'], _ = accuracy_object_level(pred_labeled, gt_labeled, False)
    if 'pq' in names:
...
def accuracy_object_level(pred, gt, hausdorff_flag=True):
    ...
    groundtruth: dice, p_iou, hausdorff """
...
def AJI_fast(gt, pred_arr):
    ...
    try:
        j = np.argmax(iou_arr, axis=1)  # get the instance number in seg who has maximum p_iou with gt
    except ValueError:
        return 0
...
def pixel_iou(gt, pred):
    ...
    p_iou = float(tp) / pred_true_union
    return p_iou
...
def get_pq(true, pred, match_iou=0.5, remap=True):
    ...
    Args:
        ...
        match_iou (float): p_iou threshold level to determine the pairing between
            GT instances `p` and prediction instances `g`. `p` and `g` is a pair
            if p_iou > `match_iou`. However, pair of `p` and `g` must be unique
            ...
            `match_iou` >= 0.5, all p_iou(p,g) > 0.5 pairing is proven to be unique and
    ...
    for true_id in true_id_list[1:]:  # 0-th is background
        ...
        for pred_id in pred_true_overlap_id:
            ...
            p_iou = inter / (total - inter)
            pairwise_iou[true_id - 1, pred_id - 1] = p_iou
    ...
    if match_iou >= 0.5:
        ...
        # paired with p_iou = 0.0 i.e no intersection => FP or FN
    ...
    # get the SQ, no paired has 0 p_iou so not impact
