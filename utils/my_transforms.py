...
def get_transforms(p):
    ...
    if 'to_tensor' in p:
        t_list.append(ToTensor(p['to_tensor']))     # p['to_tensor']的值转为tensor类型，并将其追加到t_list中

    # if 'normalize' in p:
    #     t_list.append(Normalize(mean=p['normalize'][0], std=p['normalize'][1]))     # 取出p['normalize']的第1,2个元素,作为Normalize操作的mean和std参数，然后对数据进行归一化
